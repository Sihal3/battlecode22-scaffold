...
strictfp class RunSoldier {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        MapLocation target = findtarget(rc, me, enemy);
    ...
    public static MapLocation findtarget(RobotController rc, MapLocation me, MapLocation enemy) throws GameActionException{
        ...
        if (rc.getHealth() < 20){
            target = new MapLocation(rc.readSharedArray(36)-1, rc.readSharedArray(37)-1);
            ;
        }

        //away from other soldiers
        RobotInfo[] troops = rc.senseNearbyRobots(-1, rc.getTeam());
        ...
        //find enemies around
        if(enemy != null){
            if (enemy.distanceSquaredTo(me) < 8){
                return me.subtract(me.directionTo(enemy));
            } else if (enemy.distanceSquaredTo(me) > 13){
                return enemy;
            } else {
                return target;
            }
        ...
        //check soldiertarget
        if(rc.getRoundNum() < 800 && rc.readSharedArray(34) != 0 && rc.readSharedArray(35) != 0) {
            MapLocation soldierTarget = new MapLocation(rc.readSharedArray(34) - 1, rc.readSharedArray(35) - 1);
            if(soldierTarget.isWithinDistanceSquared(me, RobotPlayer.maxFollow)){
